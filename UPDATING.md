...
- Removed the deprecated `DRILL_TO_DETAIL` feature flag. The previous default value was `True`, so drill-to-detail functionality is permanently enabled. All `isFeatureEnabled(FeatureFlag.DrillToDetail)` checks have been collapsed to their `True` branches.
