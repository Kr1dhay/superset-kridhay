...
- Removed the deprecated `ENABLE_JAVASCRIPT_CONTROLS` feature flag. The previous default value was `False`, so JavaScript in chart controls remains permanently disabled for security (XSS prevention). The deck.gl GeoJSON plugin's JavaScript-mode controls have been removed accordingly.
